...
static void update_active_limit(active_limit_t& al, int x, int y, int z, int w)
{
    if (x < al.x_min)
        al.x_min = x;
    if (x > al.x_max)
...
{
    update_active_limit(al, co.x, co.y, co.z, co.w);
}
...
void extend_cubes(std::map<coord_t, char>& cubes, active_limit_t al)
{
    ...
    for (int x = al.x_min - 1; x <= al.x_max + 1; x++)
        for (int y = al.y_min - 1; y <= al.y_max + 1; y++)
            for (int w = al.w_min - 1; w <= al.w_max + 1; w++) {
                coord_t co;
                co.x = x; co.y = y; co.w = w;
                co.z = al.z_min - 1;
                if (cubes.find(co) == cubes.end())
                    cubes[co] = '.';
                co.z = al.z_max + 1;
...
int main()
{
    ...
    for (int i = 0; i < input.size(); i++)
        for (int j = 0; j < input[0].size(); j++) {
            ...
            cubes[co] = input[i][j];
            if (input[i][j] == '#')
                update_active_limit(al, i, j, 0, 0);
        }
