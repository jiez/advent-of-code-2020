...
struct nfa_t {
    ...
    nfa_t(const std::map<int, std::vector<std::vector<int>>>& rules, int rule0_no, int recursion_limit) {
        ...
        // (rule, subrule) -> recursion_count
        std::map<std::pair<int, int>, int> recursion_count;
        ...
        while (!working.empty()) {
            ...
            for (int i = 0; i < subrules.size(); i++) {
                auto subrule = subrules[i];
                if (std::find(subrule.begin(), subrule.end(), rule_no) != subrule.end()) {
                    std::pair<int, int> rule_subrule{rule_no, i};
                    auto found = recursion_count.find(rule_subrule);
                    if (found == recursion_count.end())
                        recursion_count.insert(std::pair<std::pair<int, int>, int>{rule_subrule, 1});
                    else if (found->second < recursion_limit)
                        found->second++;
                    else
                        continue;
                }
...
static unsigned long long solution_for_puzzle_2(const std::map<int, std::vector<std::vector<int>>>& rules, int rule0_no, const std::vector<std::string>& messages, int recursion_limit)
{
    int valid_messages = 0;

    nfa_t nfa(rules, rule0_no, recursion_limit);
    dfa_t dfa(nfa);
...
int main()
{
    ...
    */
    ...
    std::cout << "It will take a while ...\n";
    // In theory we should caculate how many letters added by each recursion
    // of each recursive rule, then accordingly decide how many recursions
    // they can have. But for simplicity we just pass half of max message
    // length, which gives us the correct answer for the puzzle...
    result = solution_for_puzzle_2(rules, 1000, messages, max_msg_len / 2);
